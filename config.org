...
#+begin_src emacs-lisp
  ...
  (use-package dashboard
    :after all-the-icons
    :config
    (load "~/.emacs.d/lisp/dashboard-config.el"))
#+end_src
...
#+begin_src emacs-lisp
  ...
  (use-package bicep-ts-mode
    :config
    (with-eval-after-load 'lsp-mode
      (let ((dotnet8-path (string-trim (shell-command-to-string "mise where dotnet@8"))))
        (add-to-list 'lsp-language-id-configuration '(bicep-ts-mode . "bicep"))
        (lsp-register-client
         (make-lsp-client
          :new-connection (lsp-stdio-connection
                           `("env"
                             ,(concat "DOTNET_ROOT=" dotnet8-path)
                             ,(concat "PATH=" dotnet8-path ":" (getenv "PATH"))
                             "dotnet"
                             ,(expand-file-name "~/.dotnet/tools/bicep-langserver/Bicep.LangServer.dll")))
          :activation-fn (lsp-activate-on "bicep")
          :server-id 'bicep)))))
#+end_src
...
#+begin_src emacs-lisp
  ...
  ;; Claude Code with SSH
  (use-package claude-code
    :ensure (:type git
                   :host github
                   :repo "stevemolitor/claude-code.el"
                   :protocol ssh)
    :commands (claude-code claude-code-mode)
    :config
    (add-hook 'claude-code-process-environment-functions #'monet-start-server-function)
    (monet-mode 1)
    (claude-code-mode)
    (setq claude-code-enable-notifications t)
    ;; Display Claude in a right sidebar (33% width)
...
#+RESULTS:
: [nil 26831 57330 214363 nil elpaca-process-queues nil nil 228000 nil]
